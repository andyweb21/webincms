<x-app-layout>
    <x-slot:title>Page</x-slot:title>
    <div>
        <x-breadcrumb>
            <x-breadcrumb.item>Page</x-breadcrumb.item>
        </x-breadcrumb>

        <div id="header-content" class="flex justify-between items-center mt-4">
            <h2 class="font-semibold text-3xl">Page</h2>
            <div>
                ...
                <tbody class="text-gray-500 text-sm font-medium">
                    ...
                    <tr class="border-b border-gray-200 bg-white hover:bg-gray-100">
                        <td class="py-3 px-6 text-left">
                            <img src="{{ Storage::url('public/pages/').$page->image }}" class="rounded-md w-10 h-10 object-cover">
                        </td>
                        <td class="py-3 px-6 text-left whitespace-nowrap">
                            <a href="#" class="font-semibold text-gray-800">{{ $page->title }}</a>
                        </td>
                        <td class="py-3 px-6 text-left">
                            News & Media
                        </td>
                        ...
                        <td class="py-3 px-6 text-center">
                            ...
                            <span class="bg-green-200 text-green-600 py-1 px-3 rounded-full text-xs">{{ $page->status }}</span>
                            @else
                            <span class="bg-gray-200 text-gray-600 py-1 px-3 rounded-full text-xs">{{ $page->status }}</span>
                            @endif
                        </td>
                        <td class="py-3 px-6 text-center">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-6 text-center bg-white">Data Page belum Tersedia.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            </div>
            {{ $pages->links('vendor.pagination.webincms') }}
